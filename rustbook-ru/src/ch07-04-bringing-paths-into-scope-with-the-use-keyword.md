...
Оператор glob часто используется при тестировании для подключения всего что есть в модуль `tests`; мы поговорим об этом в разделе [«Как писать тесты»] главы 11. Оператор <code>glob</code> также иногда используется как часть шаблона автоматического импорта прелюдии: смотрите <a>стандартная библиотечная документация</a> для получения дополнительной информации об этом шаблоне.
