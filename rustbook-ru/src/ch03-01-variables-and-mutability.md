...
Константы объявляются с помощью ключевого слова `const` вместо ключевого слова `let` и тип значения *должен* быть явно указан. Мы собираемся объяснить типы и аннотации типов в следующем разделе [“Типы данных”], так что сейчас не волнуйтесь про детали. Просто знайте, что тип нужно всегда указать явно.
